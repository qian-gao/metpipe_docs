...
- Visual inspection
  ...
  - Z-score method: values with a z-score > 3 or < -3 are considered outliers.
...
- **Log transform** (often `log2` or `ln`): widely used because intensities are frequently right-skewed and effects are often multiplicative.
